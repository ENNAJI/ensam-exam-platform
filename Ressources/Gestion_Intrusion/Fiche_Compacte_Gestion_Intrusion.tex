...
Interconnexion systèmes (ARPANET → Internet académique).\\
Standardisation TCP/IP (1983).\\
Militarisation réseaux (Défense US, résilience).\\
Triade CIA: Confidentialité, Intégrité, Disponibilité.\\
Besoin confidentialité: données militaires, universitaires, bancaires.
...
\textbf{Objectif stratégique:} Réactive → Proactive (détection précoce).
...
\begin{tabular}{|l|l|}
\hline
\rowcolor{gray!20}\textbf{Type} & \textbf{Fonction} \\
\hline
...
\textbf{Pourquoi critique?} Détection ≠ protection totale, compromis peut être actif, temps réaction = impact.\\
...
\textbf{Vulnérabilité ≠ Menace ≠ Exploit:}\\
Vulnérabilité = faiblesse technique\\
Exploit = code/méthode exploitation\\
Menace = acteur/événement malveillant

\subsection*{Vulnerability Management Lifecycle (5 étapes)}
...
\textbf{Message clé:} Pont prévention-détection, réduit alertes SOC, diminue probabilité incident.

\subsection*{Sécurité Défensive vs Offensive}

\textbf{Blue Team (Défensive):} Protéger, détecter, répondre, maintenir résilience.\\
Outils: IDS/IPS, SIEM, SOC, EDR, Gestion vulnérabilités.\\
Objectif: Réduire impact attaque. KPI: MTTD/MTTR.

\textbf{Red Team (Offensive):} Simuler attaquant réel, identifier failles, tester défenses.\\
Outils: Metasploit, Nmap, Cobalt Strike, Social engineering.\\
Objectif: Découvrir faiblesses avant attaquant. KPI: Taux compromission.

\textbf{Purple Team:} Collaboration Blue + Red + Threat Intelligence.\\
Objectif: Améliorer posture sécurité globale.

\textbf{Cycle amélioration:} Red attaque → Blue détecte → Analyse écarts → MAJ règles → Renforcement.

\textbf{Message:} Cybersécurité moderne intègre les deux dans stratégie unifiée résilience.
...
\textbf{Intrusion} (tentative, bloquable) → \textbf{Incident} (succès, impact) → \textbf{Compromission} (contrôle total, persistance)
